...
// }, [])
  useEffect(() => {
      ...
      if(RiderId!=0){
        if(formattedDate=="")
      {
        fetch(URL+"/payment/suppliers_list/?delivery_person_id="+RiderId+"&date="+datee + '-' +monthh+ '-' + yearr+"&supplier_payment_status=unpaid")
      ...
      .then( async (response) => {
        ...
        fetch(URL+"/payment/suppliers_list/?delivery_person_id="+RiderId+"&date="+formattedDate+"&supplier_payment_status=unpaid")
       // /payment/suppliers_list/?delivery_person_id=25&date=12-05-2021&supplier_payment_status=unpaid
        // fetch(URL+'/client_app/clients_list/33/')
      .then( async (response) => {
          setIsLoading(true)
          let data = await response.json();
