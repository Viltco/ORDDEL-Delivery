...
function OrdersStatus({navigation,route}) {
   ...
   const [clientImage,setClientImage]=useState("");
    ...
    const purchase=()=>{
        setIsLoading(true)
        navigation.navigate("DeliveryNote",{data:boxData,dataDetail:boxDetail,OID:OId,OrderBoxId:OrderBoxId,totalQuantity:totalQuantity})
          // toggleBottomNavigationView();
          setDisvisible(true)
    }
    ...
    useEffect(() => {
        ...
        .then((responseJson) => {
              ...
              console.log("OrderBoxDetail:",responseJson);
    ...
    return (
     ...
     <View style={{paddingTop:Platform.OS=='ios'? 15:15,paddingLeft:15}}>
     <Image source={clientImage==null||clientImage==""?require('../assets/profilelogo.png'):{uri:clientImage}} 
 style={{width:Platform.OS=='ios'? 90:80,height:Platform.OS=='ios'? 90:80,borderRadius:60}} 
 />
       {/* <Image source={require('../assets/profilelogo.png')} style={{width:Platform.OS=='ios'? 90:80,height:Platform.OS=='ios'? 90:80}}  /> */}
     </View>
     
       <View style={{paddingTop:Platform.OS=='ios'? 0:0,paddingLeft:12,alignSelf:'center'}}>
       <Text style={{color:'black',fontWeight:'bold',borderBottomWidth:2,borderBottomColor:Colors.textGreyColor,fontSize:Platform.OS=='android'? 20:22,alignSelf:'center',width:230}}>{boxData.client}</Text>
       <Text style={{fontSize:13,marginTop:2}}>Order No: {boxData.purchase_order_no}</Text>
       </View>
     </View>
     </View>
     <View style={{flexDirection:'row',borderBottomColor:Colors.textGreyColor,borderBottomWidth:1,width:'90%',alignSelf:'center'}}>
        <View style={{height: 80,width: 80,borderRadius: 100,borderColor:Colors.textGreyColor,borderWidth:5,marginTop:'3%',marginBottom:10}}>
            <Text style={{textAlign:'center',color:Colors.themeColor,marginTop:"35%",fontWeight:'bold',fontSize:12}}>{boxData.order_delivery_datetime}</Text>
        </View>
        <View style={{alignSelf:'center',marginLeft:"5%"}}>
        <View style={{backgroundColor:Colors.themeColor,height:Platform.OS=="android"?17:15, borderRadius:4,width:"40%"}}>
      <Text style={{color:"white",fontSize:12,fontWeight:'bold',textAlign:'center',}}>{dataStatus.toUpperCase()}</Text>
      </View>
            <Text style={{color:Colors.productGrey,fontSize:14}}>Delivery Address: </Text>
            <Text style={{fontSize:16,width:210}}>{boxData.business_address}</Text>
        </View>
    ...
    {/* <View style={{borderBottomColor:Colors.textGreyColor,borderBottomWidth:2,}}> */}
        ...
        {deliveryNote==""?null:<Card style={{width:"90%",alignSelf:'center',backgroundColor:"#E2E2E2",elevation:0,marginTop:20,padding:10}}>
          <Text style={{color:Colors.productGrey,textAlign:'center'}}>Note: {deliveryNote}</Text>
        </Card>}
        {dataStatus=="in progress"?null:
        <View style={{alignSelf:'center',marginTop:20}}>
              {dataStatus=='in progress'||dataStatus=="purchased"?null:<View><TouchableOpacity style={styles.button} disabled={dataStatus=="in progress"||dataStatus=="purchased"?true:false} onPress={accept}>
        ...
        >
          ...
          <View style={styles.bottomNavigationView}>
              ...
              {dataStatus=='in progress'?null:<TouchableOpacity style={styles.button} disabled={dataStatus=="in progress"||dataStatus=="purchased"?true:false} onPress={accept}>
                <Text style={styles.buttonText}>ACCEPTED</Text>
            </TouchableOpacity>}
            {dataStatus=='pending'?null:
            <TouchableOpacity disabled={dataStatus=="purchased"||dataStatus=="pending"?true:false} onPress={purchase} style={styles.button}>
