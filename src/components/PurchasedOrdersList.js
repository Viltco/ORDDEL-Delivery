...
const PurchasedOrdersList = ({ navigation, route }) => {
...
//   setDisvisible(true)
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    if(RiderId!=0){
      fetch(URL+"/order/list_assigned_orders/?delivery_person_id="+RiderId+"&choice=purchased")
      ...
      .then( async (response) => {
          ...
          //   console.log("status code",data)
            if(response.status==200){
              console.log("Pending:",data.response.status);
  ...
  return (
    <View style={{ flex:1,backgroundColor:'white'}}>
        {/* <MyHeader name="PURCHASED ORDERS" nav={navigation}/> */}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            onPress={()=>{
                ...
                //     }
                //   });
                navigation.navigate("PurchasedOrderStatus",{OrderBox:item.order_box,OrderId:item.id,Packages:item.no_of_items,Quantity:item.total_quantity});
            }}
